...
var guestroomno = 0;
baseURL="http://" + middleware_ip + "/"+Projctfldr+"/"
...
function fngetroomfromwebapi() {
    $.ajax({
        type: "POST",
        url: "http://" + middleware_ip + "/"+Projctfldr+"/api_getroom.php?tvduid="+getDuid(),
            ...
            .then(data => {
                ...
                if (json_data_count != '0') {
                    ...
                    if (data.errorcode == '1') {
                        ...
                        var welcomeMsg = '<span style="color:#ffe300" class="d-inline-block">&nbsp;' + uppertolwercase(guestData.title) + '&nbsp;' + uppertolwercase(guestData.lname) + '</span>';
...
function uppertolwercase(str){  
    var str_rtn=str.toLowerCase().replace(/\b[a-z]/g, function(letter) {
