...
var getPropValueArray = function() {
 ...
 function success1CB(videoSource) {
	// document.getElementById("logtext").innerHTML += "<br>videoSource " + videoSource;
...
var launchSource = function(sourceName) {
	...
	var connectedVideoSources;
	 function successCB(source, type) {
		setState('1');
		// document.getElementById("logtext").innerHTML += "<br> setSource() is successfully done. source name = " + source.name + ", source port number = " + source.number;
	 ...
	 function errorCB(error) {
		// document.getElementById("logtext").innerHTML += "<br> setSource() is failed. Error name = "+ error.name + ", Error message = " + error.message;
	 ...
	 function systemInfoSuccessCB(videoSource) {
	    ...
	    //  }
	     for (var i = 0; i < connectedVideoSources.length; i++) {
				// document.getElementById("logtext").innerHTML += "<br>--------------- Source " + i + " ---------------";
				// document.getElementById("logtext").innerHTML += "<br>type = " + connectedVideoSources[i].type;
				// document.getElementById("logtext").innerHTML += "<br>number = " + connectedVideoSources[i].number;
	    	 console.log("--------------- Source " + i + " ---------------");
	         console.log("type = " + connectedVideoSources[i].type);
	         console.log("number = " + connectedVideoSources[i].number);
	    	 if (connectedVideoSources[i].type === sourceName && connectedVideoSources[i].number==2) {
		...
		setTimeout(chksrchdmin2, 1000);
	 }
	function chksrchdmin2(){		
		if (resource_status===false){			
			show_msg_guest('Unable to find source right now. Kindly contact hotel support for further assistance.');
...
function handleBack() {
	hideSource();
	resource_status=false;
	// window.location = "tv-channels.html?fromurl=scrn_cast";
